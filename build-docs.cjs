...
adocFiles.forEach((adocFile) => {
  ...
  const markdownContent = asciidoctor.convert(adocContent, {
    safe: 'safe',
    standalone: true,
  });
  ...
  const images =
    adocContent.match(/image::(.*?)\[/g) || adocContent.match(/image:(.*?)\[/g); // check if there are any images (TODO: maybe check for other imports in the future)
